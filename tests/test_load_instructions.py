...
class ProcessorTest(unittest.TestCase):
    ...
    def test_ldx_absolute_y_same_page():
        test_load_instruction(instruction=[LDX_ABSOLUTE_Y, 0x00, 0x30], register='X', value=0xf0, num_cycles=4,
                              data={0x3050: 0xf0}, register_data={'Y': 0x50})
    ...
    def test_ldy_immediate():
        test_load_instruction(instruction=[LDY_IMMEDIATE, 0xff], register='Y', value=0xff, num_cycles=2)

    @staticmethod
    def test_ldy_zero_page():
        test_load_instruction(instruction=[LDY_ZERO_PAGE, 0xaa], register='Y', value=138, num_cycles=3, 
                              data={0x00aa: 138}, register_data={'Y': 0x20})

    @staticmethod
    def test_ldx_zero_page_y():
        test_load_instruction(instruction=[LDY_ZERO_PAGE_X, 0xF0], register='Y', value=42, num_cycles=4, 
                              data={0x0010: 42}, register_data={'X': 0x20})

    @staticmethod
    def test_ldy_absolute():
        test_load_instruction(instruction=[LDY_ABSOLUTE, 0x10, 0x20], register='Y', value=0xf0, num_cycles=4,
                              data={0x2010: 0xf0})

    @staticmethod
    def test_ldy_absolute_x_same_page():
        test_load_instruction(instruction=[LDY_ABSOLUTE_X, 0x00, 0x30], register='Y', value=0xf0, num_cycles=4,
                              data={0x3050: 0xf0}, register_data={'X': 0x50})

    @staticmethod
    def test_ldy_absolute_x_cross_page_boundary():
        test_load_instruction(instruction=[LDY_ABSOLUTE_X, 0xf0, 0x30], register='Y', value=42, num_cycles=5,
                              data={0x3180: 42}, register_data={'X': 0x90})
