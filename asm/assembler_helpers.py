...
ADDRESS_MODE_PATTERNS = {
    'A': 'Accumulator',
    '\#\$[0-9A-F]{2}': 'Immediate',
    '\$[0-9A-F]{2}': 'Zero Page', '\$[0-9A-F]{2},X': 'Zero Page,X',
    '\$[0-9A-F]{2},Y': 'Zero Page,Y',
    '\$[0-9A-F]{4}': 'Absolute',
    '\$[0-9A-F]{4},X': 'Absolute,X',
    '\$[0-9A-F]{4},Y': 'Absolute,Y',
    '\(\$[0-9A-F]{4}\)': 'Indirect',
    '\(\$[0-9A-F]{2},X\)': 'Indirect,X',
...
ADDRESS_MODE_PATTERNS_SYM = {
    ...
    '[0-9A-Z+-]{1,8}': ('Zero Page', 'Absolute'),
    '[0-9A-Z]{1,8},X': ('Zero Page,X', 'Absolute,X'),
    '[0-9A-Z]{1,8},Y': ('Zero Page,Y', 'Absolute,Y'),
    '\([0-9A-Z]{1,8}\)': 'Indirect',
    '\([0-9A-Z]{1,8},X\)': 'Indirect,X',
    '\([0-9A-Z]{1,8}\),Y': 'Indirect,Y',
...
def determine_mode(mnemonic, operand):
    ...
    good_mnemonic = False
    for m, opc in ADDRESS_MODES[mode][1]:
        if m == mnemonic:
            good_mnemonic = True
            break
    if good_mnemonic:
