...
def test_push_accumulator_instruction(instruction: list[int], num_cycles: int, data: dict = None,
...
def test_pull_accumulator_instruction(instruction: list[int], value, num_cycles: int, data: dict = None,
                                      register_data: dict = None) -> None:
    processor = setup_processor(instruction, data=data, registers=register_data)
...
def test_push_processor_status_instruction(instruction: list[int], num_cycles: int, data: dict = None,
                                           register_data: dict = None, flags: dict = None) -> None:
    processor = setup_processor(instruction, data=data, registers=register_data, flags=flags)

    status = processor.C + (processor.Z << 1) + (processor.I << 2) + (processor.D << 3) + (processor.B << 4) + (
                processor.V << 6) + (processor.N << 7)
    sp = processor.S
    processor.run_instruction()
...
def test_pull_processor_status_instruction(
        instruction: list[int], num_cycles: int, data: dict = None, register_data: dict = None,
...
class ProcessorTest(unittest.TestCase):
    ...
    @staticmethod
    def test_push_processor_status():
